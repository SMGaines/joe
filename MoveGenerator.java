...
public class MoveGenerator 
{
	...
	MoveGenerator()
	{
		moveList = new Move[Joe.maxSearchDepth][maxMoves];
		numMoves = new int[Joe.maxSearchDepth];
		tempMove = new Move();
		
		for (int i=0;i<Joe.maxSearchDepth;i++)
		{
	...
	void generateMoveList(Board b,int depth,int colour, boolean aCapturesOnly)
	{
	...
	int getNumMoves(int depth)
	{
		return numMoves[depth];
	}
	 
	void setScore(int depth,int index,int aScore)
	{
		moveList[depth][index].setScore(aScore);
	}
	 
	Move getMove(int depth,int index)
	{
		return moveList[depth][index];
	}
	 
	void swap(int depth,int m1,int m2)
	{
